import OrderRow, { Order } from "./OrderRow";
import React, { useState } from 'react';
import CreateOrderModal, { CreateOrderPayload } from "./ModalOrder";

const columns = ["nb commande", "Nom du produit", "Prix unit.", "Quantité", "Total"];

interface RecentOrdersProps {
  orders: Order[];
  onCreateOrder?: (payload: CreateOrderPayload) => Promise<void>;
  onOrderClick?: (order: Order) => void;  // ← ajoute ça pour la modal détail
}

export default function RecentOrders({ orders, onCreateOrder, onOrderClick }: RecentOrdersProps) {
  const [isModalOpen, setIsModalOpen] = useState(false);

  return (
    <>
      <div className="bg-white rounded-2xl p-6 shadow-sm">
        {/* Header */}
        <div className="flex items-center justify-between mb-5">
          <span className="text-sm font-semibold text-gray-900 tracking-tight">
            Commande Recents
          </span>
          <button
            onClick={() => setIsModalOpen(true)}
            className="flex items-center gap-1.5 bg-orange-700 hover:bg-orange-600 text-white text-xs font-medium px-3 py-1.5 rounded-lg transition-colors"
          >
            + Créer une commande
          </button>
        </div>

        {/* Table */}
        <table className="w-full border-collapse">
          <thead>
            <tr>
              {columns.map((col) => (
                <th key={col} className="text-left pb-3 px-3 border-b border-gray-300">
                  <button className="inline-flex items-center gap-1 text-xs font-medium text-gray-600 hover:text-gray-800 transition-colors">
                    {col}
                    <span className="text-[10px]">▼</span>
                  </button>
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {orders.map((order) => (
              <OrderRow
                key={order.id}
                order={order}
                onClick={() => onOrderClick?.(order)}  // ← branche le click
              />
            ))}
          </tbody>
        </table>
      </div>

      {/* Modal créer — une seule fois, en dehors du div */}
      <CreateOrderModal
        isOpen={isModalOpen}
        onClose={() => setIsModalOpen(false)}
        onSubmit={async (payload) => {
          await onCreateOrder?.(payload);
        }}
      />
    </>
  );
}